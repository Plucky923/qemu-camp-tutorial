...
## 9. 为什么 lifter 修不好，也不值得修

这个 lifter 是修不好的，也不值得修。真实场景下的 CUBIN 里嵌有对应 PTX 的概率可能很高（标准库有，llama.cpp 带出来的 CUBIN 里也有），纯二进制的情况或许是少数。ptx 到 llvm ir 到 AMD 后端这一步有 zluda 背书，问题应该不大。其他计算卡呢？tmatmul 后端直接消费的是 ptx，具体代码实现也是一言难尽——就算修好了，后面的编译管线也是堵死的。
...
!!! note "AI 文档的固有缺陷"
    ...
    一时就想到这两点，剩下的问题亦难以尽述。希望 AI 文档的作者能注意到这一点，对于关键文档至少多一层润色，多一层审计，别把工作量甩给下游。
...
TVM<sup>[20]</sup> 的 Relay → TIR → 目标代码是一条经过广泛验证的编译管线。同一份计算图，target="cuda" 产出的 PTX 用 ptxas<sup>[21]</sup> 编译，target="llvm" 产出的 CPU 代码用 Clang 编译——两边的输出应该完全一致。这个方法可以直接用于 CXL 验证：同样的 Qwen 算子，分别在 host native、guest VFIO、guest CXL 内存三条路径上执行，逐条比对。
...
模型选择也要换。Kimi K2.6（IQ1_M 量化后仍有 200+ GB）换成 Qwen1.5-MoE-A2.7B 量化版（3-4 GB），或直接用更小的 dense 模型。这个选择更务实，不需要数据中心级的显卡，消费级显卡就可以。debug 循环从 8-16 分钟变成秒级，可以真正做迭代式开发。
