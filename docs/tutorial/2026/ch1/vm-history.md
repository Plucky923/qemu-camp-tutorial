...
20 世纪 90 年代末，随着 x86 架构通用计算机的广泛普及，虚拟化技术的应用边界得到显著拓展。VMware Workstation 的发布，标志着业界首个成熟的 x86 架构商业虚拟化解决方案正式问世。针对当时 x86 处理器缺乏硬件辅助虚拟化支持的技术限制，该方案创新性地采用了**二进制翻译（Binary Translation）**技术——即在运行时将客户机的机器指令逐条或逐块地转换为宿主机可以直接执行的等价指令序列——成功实现了指令级的模拟与转换。这一突破性进展不仅攻克了 x86 平台的虚拟化难题，更标志着虚拟化技术正式从专用服务器领域下沉至桌面计算环境。
...
然而，这种纯软件定义的虚拟化技术存在明显的性能瓶颈。由于必须依赖二进制翻译机制来捕获和替换特权指令，且需在软件层面维护**影子页表（Shadow Page Tables）**——由 VMM 额外维护的一份页表，用于将客户机的虚拟地址直接映射到宿主机物理地址，从而代替硬件完成两级地址翻译——以实现内存地址转换，这导致了大量的 CPU 开销和上下文切换损耗，特别是在 I/O 密集型场景下性能下降显著。
...
以上脉络围绕的都是"同一架构上跑更多操作系统"的问题。但虚拟化还有另一个维度——跨越硬件架构的边界：在一种处理器上运行为另一种处理器编写的软件。这就催生了模拟器和跨平台技术的发展。
...
前面两条脉络分别关注"资源利用效率"和"跨平台兼容性"，但对于处理器设计者和体系结构研究者来说，还有第三类需求——在芯片流片之前，用软件精确模拟 CPU 内部的流水线、缓存和互连行为，从而评估和验证新的微架构设计方案。这就是微架构仿真的由来。
...
回顾上面三条发展脉络，可以看到虚拟化技术在不同领域催生出了丰富的应用形态。下面我们将这些典型场景汇总成一张表格，帮助你建立全景认知：
