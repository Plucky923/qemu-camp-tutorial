...
RC 和 EP 的交互依赖于正确的地址翻译——CPU 侧地址和 PCIe 侧地址往往不同。iATU 正是负责这一翻译工作的硬件单元。
...
理解了 PCIe 的角色划分和地址翻译机制后，我们来看几个典型的应用场景，了解 PCIe 在实际系统中扮演的角色。
...
以上是真实硬件中的 PCIe 应用。回到 QEMU，我们来看它是如何在软件层面组织 PCIe 拓扑的。
...
BAR（Base Address Register，基地址寄存器）的本质是“设备向系统申请一段地址空间”，在 QEMU 中通过 `MemoryRegion` 绑定到 PCI 设备：
