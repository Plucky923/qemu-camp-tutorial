...
MTTCG 解决了多核并行的问题，但每个 vCPU 线程上翻译块之间的上下文切换开销依然存在。接下来我们看 TCG 如何通过直接跳转来减少这部分开销。
...
直接跳转让 TB 之间的执行更高效，但所有翻译出的 TB 都需要一个统一的存储区域来管理。这就是代码缓存的作用。
...
以上介绍了 TCG 的核心翻译与执行机制。在此基础上，QEMU 还提供了一套插件接口，允许用户在不修改 QEMU 源码的情况下对客户机代码进行插桩分析。
